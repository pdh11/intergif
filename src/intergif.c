...
static BOOL ChoosePaletteMapper( const char *infile, BOOL bList, BOOL bJoin,
                                 BOOL b216, BOOL b256, BOOL g16, BOOL g256, const char *palfile,
                                 int nBest, palettemapper *pMapper )
{
    ...
    {
        pmap = PaletteMapper_To16g();
    }
    else if ( g256 )
    {
        pmap = PaletteMapper_To256g();
    }
    else if ( palfile )
    {
	...
	if ( bList )
	    result = Anim_ConvertList( infile, NULL, &Histogrammer, NULL );
        else if ( bJoin )
               ...
               const char *outfile )
{
    ...
    /* when mapping to 256 grey levels, there is no point in error diffusion */
    if ( g256 )
        bDiffuse = FALSE;
	...
	if ( bList )
	{
            Anim_SetError( "-cfsi cannot be used with -list" );
            return FALSE;
	}
    ...
    if ( !ChoosePaletteMapper( infile, bList, bJoin, b216, b256, g16, g256, palfile, nBest,
                               &pmap ) )
        return FALSE;

    a = bList? (Anim_FromList( infile, pmap, bSame, bDiffuse, bZigZag, g16 | g256 ))
             : bJoin ? Anim_FromFiles( infile, pmap, bSame, bDiffuse, bZigZag, g16 | g256 )
                     : Anim_FromFile( infile, pmap, bSame, bDiffuse, bZigZag, g16 | g256 );
    ...
    for ( i=0; i < a->nFrames; i+= nFramesPerFile )
    {
        ...
        if ( bSprite )
            result = Anim_SaveSprite( a, output, i, i + nFramesPerFile-1, bNewFormat, xdpi, ydpi );
        else
